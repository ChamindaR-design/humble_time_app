...
import 'package:humble_time_app/shared_prefs/save_reflection.dart' as reflect;
...
class _PacingScreenState extends State<PacingScreen> with TickerProviderStateMixin {
  ...
  bool hasNavigatedToSummary = false;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        VoiceService.speak("Block started. Let’s stay present and keep pacing yourself.");
  ...
  void _setupTimerController() {
    ...
    _timerController.addStatusListener((status) async {
      if (status == AnimationStatus.completed) {
        ...
        await VoiceService.speak("Block ${currentBlock + 1} completed.");
        setState(() => isRunning = false);

        if (affirmationsEnabled && currentBlock + 1 < totalBlocks) {
          await Future.delayed(const Duration(milliseconds: 300));
          await VoiceService.speak(_getAffirmationForIntention());
        }

        await Future.delayed(const Duration(milliseconds: 300));
        await VoiceService.speak("What would you like to do next?");

        if (currentBlock + 1 < totalBlocks) {
          await Future.delayed(const Duration(milliseconds: 500));
          setState(() => currentBlock++);
          _timerController.duration = Duration(seconds: blockDurationSeconds);
          _timerController.reset();
          _timerController.forward(from: 0);
          setState(() => isRunning = true);
          await Future.delayed(const Duration(milliseconds: 300));
          await VoiceService.speak("Beginning block ${currentBlock + 1}. Let’s breathe together.");
        } else {
          if (hasNavigatedToSummary || !mounted) return;
          hasNavigatedToSummary = true;

          await Future.delayed(const Duration(milliseconds: 500));
          await VoiceService.speak("All pacing blocks complete. Well done.");
  ...
  void startBlock() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        VoiceService.speak("Beginning block ${currentBlock + 1}. Let’s breathe together.");
      }
    });
  }
  ...
  void saveReflection() {
    reflect.saveReflection(
      currentBlock: currentBlock,
      intention: selectedIntention.name,
      breathingStyle: selectedBreathing.name,
      note: "User exited session from pacing screen.",
    );
  }
            ...
            showDialog(
              ...
              builder: (BuildContext dialogContext) {
                return AlertDialog(
                  ...
                  content: const Text("Would you like to save a reflection before exiting?"),
                  actions: [
                    TextButton(
                  ...
                  child: Column(
                    ...
                    children: [
                      DropdownButton<int>(
                        ...
                        items: [60, 180, 300, 600, 900, 1500].map((sec) {
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          isRunning
                              ? ElevatedButton(
                                  onPressed: stopBlock,
                                  child: Text("Pause", style: TextStyle(fontSize: fontSize)),
                                )
                              : ElevatedButton(
                                  onPressed: startBlock,
...
class SessionEndScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<Reflection?>(
      future: reflect.loadLatestReflection(),
